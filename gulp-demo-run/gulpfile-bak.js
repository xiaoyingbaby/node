...
gulp.task('clean', function(){
	gulp.src(['./dist/html','./dist/js','./dist/css','./dist/img'])
		.pipe($.clean());
});
...
gulp.task('sprite',function(){
    gulp.src('./src/img/icon/*')
        .pipe($.spritesmith({
            ...
            cssName:'src/css/sprite.css',   //保存合并后对于css样式的地址
...
//任务9：打包任务组,大改动/第一次生成发布时用，同步执行/队列执行
//gulp任务是异步的，task写法上可以省略return，但在需要同步执行任务时，这个return是不能省略的。
